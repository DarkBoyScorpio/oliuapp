...
SHARE_URL = st.secrets.get("SHARE_URL") or os.getenv("SHARE_URL")
GSP_CRED = st.secrets.get("GSP_CRED") or os.getenv("GSP_CRED")
...
if menu == "📥 Nhập đơn hàng":
    ...
    st.markdown("Vui lòng điền các thông tin bên dưới. Sau đó ấn Gửi đơn hàng")
    with st.form("form_nhap_don"):
        # ==== PHẦN 1: Thông tin khách hàng ====
        with st.expander("👤 Thông tin khách hàng", expanded=False):
        ...
        with st.expander("🍱 Snack - Mít, Chuối, Khoai, Cơm cháy", expanded=False):
            col1, col2 = st.columns(2)
            with col1:
                mit_500g = st.number_input("🥭 Mít 500g", min_value=0, step=1)
                chuoi_250g = st.number_input("🍌 Chuối mộc 250g", min_value=0, step=1)
                ktrb_250g = st.number_input("🥔 Khoai rong biển 250g", min_value=0, step=1)
                ktmam_250g = st.number_input("🥔 Khoai mắm 250g", min_value=0, step=1)
                ...
                com_chay_200g = st.number_input("🍚 Cơm cháy 200g", min_value=0, step=1)
            ...
            with col2:
                ...
                chuoi_500g = st.number_input("🍌 Chuối mộc 500g", min_value=0, step=1)
                ktrb_500g = st.number_input("🥔 Khoai rong biển 500g", min_value=0, step=1)
                ktmam_500g = st.number_input("🥔 Khoai mắm 500g", min_value=0, step=1)
    ...
    if submitted:
        ...
        if df_filtered.empty:
            ...
            def show_data():
                ...
                danh_sach_san_pham = [
                    "MÍT 500G", "THẬP CẨM 500G", "CHUỐI SẤY MỘC 250G", "CHUỐI SẤY MỘC 500G",
                    "KHOAI TÂY RONG BIỂN 250G", "KHOAI TÂY RONG BIỂN 500G",
                    "KHOAI TÂY MẮM 250G", "KHOAI TÂY MẮM 500G",
                    "KHOAI MÔN TRỨNG CUA 250G", "KHOAI MÔN TRỨNG CUA 500G",
                    "NẾP CHÁY CHÀ BÔNG 150G x3", "NẾP CHÁY CHÀ BÔNG 150G x5",
                    "CƠM CHÁY CHÀ BÔNG 200G", "GẠO LỨT RONG BIỂN 200G",
                    "BÁNH TRÁNG MẮM", "MẬT ONG 500ML", "MẬT ONG 1 LÍT",
                    "MẮM 1 LÍT", "ĐIỀU RANG MUỐI 200G", "ĐIỀU RANG MUỐI 500G", "ĐIỀU MẮM ỚT 500G"
                ]

                thong_tin_dat_hang = {}
                mon_hang_da_mua = {}
                for k, v in filtered_data.items():
                    if k.strip() in danh_sach_san_pham and float(v) > 0:
                        mon_hang_da_mua[k] = v
                    elif k not in danh_sach_san_pham:
